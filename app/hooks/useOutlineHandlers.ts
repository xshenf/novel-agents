...
// ═══════════════════════════════════════════════════════════════════════════════
// Standalone helper functions (pure / side-effect-free)
// ═══════════════════════════════════════════════════════════════════════════════

/** Build the AI prompt for regenerating a single chapter's beat outline. */
function buildRegenChapterPrompt(params: {
  projectTitle: string;
  ...
  const { projectTitle, chapter, flatChapters, globalIdx, worldSetting, coreConflict } = params;
  const adjacentContext = flatChapters
    .map((s, sIdx) => (sIdx !== globalIdx ? `- ${s.title}: ${s.content}` : ''))
    .filter(Boolean)
    .slice(Math.max(0, globalIdx - 2), globalIdx + 3)
    .join('\n');

  return `你是一个资深网络小说剧情策划。请为我的小说《${projectTitle}》重新规划设计【${chapter.title}】的详细章节细纲。

【当前小说设定】:
- 书名: ${projectTitle}
- 简介: ${worldSetting || '暂无'}
- 题材/核心冲突: ${coreConflict || '暂无'}
...
/** Apply the AI-regenerated chapter back into the sections array, pushing an undo entry. */
function applyRegenChapterResult(params: {
  parsedRegen: OutlineVolume[];
  originalChapter: OutlineChapter;
  volIdx: number;
  chapIdx: number;
  localSections: OutlineVolume[];
  setLocalSections: Dispatch<SetStateAction<OutlineVolume[]>>;
  ...
  const mergedCh: OutlineChapter = {
    ...newCh,
    title: originalChapter.title,
    isLocked: originalChapter.isLocked,
  };
  const savedOriginalChapter = originalChapter;
...
/**
 * Intelligently merge a newly generated outline into the existing one,
 * preserving locked chapters and their volume assignments.
 */
function mergeOutlineSections(
  newSections: OutlineVolume[],
  oldSections: OutlineVolume[],
  fallbackVolumeTitle: string,
): OutlineVolume[] {
  // Flatten old outline: preserve lock state and volume index
  ...
  // Cross-merge: keep old locked chapters in place, replace unlocked with new
  ...
  // Append excess locked chapters from old outline
  ...
  // Rebuild tree: clear old volumes and refill from merged flat list
  ...
  mergedFlat.forEach((ch) => {
    ...
    while (targetVolIdx >= mergedVolumes.length) {
      mergedVolumes.push({ title: fallbackVolumeTitle, content: '', chapters: [] });
    }
    if (targetVolIdx < 0) {
      targetVolIdx = 0;
    }
...
export function useOutlineHandlers(params: OutlineHandlersParams): OutlineHandlers {
  ...
  // ─── Shared: renumber + sync markdown ──────────────────────────────────────

  const syncSections = useCallback(
    (sections: OutlineVolume[]) => {
  ...
  const {
    ...
    handleMoveVolume,
    handleDeleteVolume,
  } = useVolumeCore({ store, localSections, onSectionsChange: syncSections });

  // ─── CRUD: Volume / Chapter Insert & Move ────────────────────────────────

  /** 添加新分卷 */
  const handleInsertVolume = useCallback(
    (volIdx: number) => {
  ...
  const handleDeleteChapter = useCallback(
    (volIdx: number, chapIdx: number) => {
      store.showConfirm('确定要删除本章节大纲吗？此后序号会自动全书重新递增。', () => {
        ...
        if (
          editingChapterPath &&
          editingChapterPath.volIdx === volIdx &&
          editingChapterPath.chapIdx === chapIdx
        ) {
  ...
  const handleMoveChapter = useCallback(
    (volIdx: number, chapIdx: number, direction: 'up' | 'down') => {
      ...
      const newSections = localSections.map((vol, vIdx) => {
        if (vIdx !== volIdx) return vol;
        const newChapters = [...vol.chapters];
        const tmp = newChapters[chapIdx];
        newChapters[chapIdx] = newChapters[targetIdx];
        newChapters[targetIdx] = tmp;
        return { ...vol, chapters: newChapters };
      });
      syncSections(newSections);
    },
  ...
  /** AI 智能重写单章 Beat */
  const handleAiRegenChapter = useCallback(
    async (volIdx: number, chapIdx: number) => {
      if (!store.currentProject) return;
      aiAbortRef.current?.abort();
      const controller = new AbortController();
      aiAbortRef.current = controller;
      const globalIdx = flatChapters.findIndex(
        (ch) => ch.volIdx === volIdx && ch.chapIdx === chapIdx,
      );
      setRegeningIndex(globalIdx);
      try {
        const sec = localSections[volIdx].chapters[chapIdx];
        const prompt = buildRegenChapterPrompt({
          projectTitle: store.currentProject.title,
          chapter: sec,
          flatChapters,
          globalIdx,
          worldSetting: tempWorldSetting || store.currentProject.worldSetting || '',
          coreConflict: tempCoreConflict || store.currentProject.coreConflict || '',
        });
        ...
        const parsedRegen = parseStructureOutline(data.reply);
        const applied = applyRegenChapterResult({
          parsedRegen,
          originalChapter: sec,
          volIdx,
          chapIdx,
        ...
        if (e.name !== 'AbortError') {
          showNotification('AI 单章重写失败: ' + e.message, 'error');
        }
      } finally {
    ...
    [
      store, callAIApi, aiAbortRef, flatChapters, localSections,
      setLocalSections, setTempOutlineFull, setRegeningIndex, pushAiUndo,
      tempWorldSetting, tempCoreConflict,
    ],
  );

  /** AI 推演分卷走向大纲 */
  const handleAiRegenVolume = useCallback(
    async (volIdx: number, userHint?: string) => {
      if (!store.currentProject) return;
      aiAbortRef.current?.abort();
      const controller = new AbortController();
      aiAbortRef.current = controller;
...
请直接输出推荐的【${vol.title}】的卷概要走向描述（字数在150字到250字之间），不需要输出任何标题、多余的说明前言或分析，直接给出描述即可。`;
        ...
        if (e.name !== 'AbortError') {
          showNotification(`AI 推演分卷走向失败: ` + e.message, 'error');
        }
      } finally {
        if (aiAbortRef.current === controller) {
          aiAbortRef.current = null;
    ...
    [
      store, callAIApi, aiAbortRef, localSections, setLocalSections,
      setTempOutlineFull, setRegeningVolumeIdx, pushAiUndo,
      tempWorldSetting, tempCoreConflict, tempPowerSystem,
    ],
  );

  // ─── Editing Save Operations ─────────────────────────────────────────────
  ...
  // ─── Recommended Outline Selection ───────────────────────────────────────

  /** 选用推荐大纲时执行层次化智能合并 */
  const handleSelectRecommendedOutline = useCallback(
    async (opt: { description?: string }) => {
      const optDesc = opt.description || '';
      const newSections = parseStructureOutline(optDesc);
      const mergedVolumes = mergeOutlineSections(newSections, localSections, '新分卷');
      ...
      if (store.currentProject) {
        try {
          ...
          showNotification('已选用新推荐大纲并完成层次合并，锁定的章节和大纲卷归属已完好保留！');
        } catch {
          showNotification('推荐大纲合并保存失败', 'error');
        }
      }
    },
  ...
  const lockedChapters = useMemo(
    () =>
      localSections.reduce(
        (sum, vol) => sum + (vol.chapters ? vol.chapters.filter((ch) => ch.isLocked).length : 0),
        0,
      ),
    [localSections],
  );
  ...
  /** 世界规则筛选映射：Record 查表替代 if-return 链 */
  const getFilteredRules = useCallback(
    (material: string) => {
      if (!store.worldRules) return [];

      const RULE_FILTER_MAP: Record<string, (rules: any[]) => any[]> = {
        location: (rules) => rules.filter((r: any) => r.type === 'location'),
        faction: (rules) => rules.filter((r: any) => r.type === 'faction'),
        item: (rules) => rules.filter((r: any) => r.type === 'item'),
        currency: (rules) => rules.filter((r: any) => r.type === 'rule' && r.name.includes('货币')),
        skillSystem: (rules) =>
          rules.filter(
            (r: any) =>
              r.type === 'rule' &&
        ...
        timeline: (rules) => rules.filter((r: any) => r.type === 'rule' && r.name.includes('时间线')),
        foreshadow: (rules) => rules.filter((r: any) => r.type === 'other' && r.name.includes('伏笔')),
        plot: (rules) =>
          rules.filter(
            (r: any) => r.type === 'other' && (r.name.includes('情节') || r.name.includes('脉络')),
          ),
        subPlot: (rules) => rules.filter((r: any) => r.type === 'other' && r.name.includes('支线')),
        events: (rules) => rules.filter((r: any) => r.type === 'other' && r.name.includes('事件')),
        relation: (rules) => rules.filter((r: any) => r.type === 'other' && r.name.includes('关系')),
      };

      const filter = RULE_FILTER_MAP[material];
      return filter ? filter(store.worldRules) : store.worldRules;
    },
    [store.worldRules],
  );
