    ...
    <>
      ...
      <div className={`drawer-content ${showSettings ? 'active' : ''}`}>
        ...
        <div className="drawer-body">
          {/* TAB 1: 模型池管理 */}
          {settingsTab === 'models' && <ModelPoolSettings />}

          {/* TAB 2: 智能体模型绑定 */}
          {settingsTab === 'bindings' && <AgentModelBinding />}

          {/* TAB 3: 全局创作提示词 */}
          {settingsTab === 'prompts' && <GlobalPromptSettings />}
        </div>
...
// ---------------------------------------------------------------------------
// ModelPoolSettings — 模型池列表 + 模型编辑/新建表单
// ---------------------------------------------------------------------------

function ModelPoolSettings() {
  ...
  const {
    ...
    handleAddNewModel, handleEditModel, handleSaveModel,
  } = models;
...
// ---------------------------------------------------------------------------
// AgentModelBinding — 智能体模型绑定与参数覆盖
// ---------------------------------------------------------------------------

function AgentModelBinding() {
  const { store, models } = useWorkspace();
  const { agentsList } = models;
